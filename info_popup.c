  ...
  if (!infWindow)
    {
      ...
      gtk_signal_connect (GTK_OBJECT (infWindow), "destroy",
			  GTK_SIGNAL_FUNC (gtk_widget_destroyed), &infWindow);
      ...
      gtk_signal_connect (GTK_OBJECT (info_list), "select_row",
			  GTK_SIGNAL_FUNC (info_selectionCB), NULL);
      ...
      gtk_signal_connect_object (GTK_OBJECT (button), "clicked",
				 GTK_SIGNAL_FUNC (info_prevCB), NULL);
      ...
      GTK_WIDGET_UNSET_FLAGS (button, GTK_CAN_FOCUS);
      gtk_signal_connect_object (GTK_OBJECT (button), "clicked",
				 GTK_SIGNAL_FUNC (info_nextCB), NULL);
      ...
      gtk_toggle_button_set_state (GTK_TOGGLE_BUTTON (radio),
				   (globals.
				    do_highlight & DO_HIGHLIGHT) ? TRUE :
				   FALSE);
      ...
      gtk_toggle_button_set_state (GTK_TOGGLE_BUTTON (radio),
				   (globals.
				    do_highlight & DO_HIGHLIGHT_DUPS) ? TRUE :
				   FALSE);
      gtk_widget_show (radio);
...
void
info_selectionCB (GtkWidget * clist,
...
void
info_prevCB (GtkWidget * widget, gpointer data)
{
  info_prev ();
}

void
info_nextCB (GtkWidget * widget, gpointer data)
{
  info_next ();
}

void
info_highlightCB (GtkWidget * widget, gpointer data)
{
  struct IMAGE_INFO *inf;
...
void
info_highlight_dupsCB (GtkWidget * widget, gpointer data)
{
...
void
clear_all_highlights (int clear_current)
{
  int x, y;
  struct IMAGE_INFO *inf;
