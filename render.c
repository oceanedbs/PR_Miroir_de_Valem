...
void
average_image_area (struct PIX *avg, GdkPixbuf * im, int x, int y, int w,
		    int h)
{
  int i, j;
  int r, g, b;
...
double
calc_stddev (GdkPixbuf * im, int x, int y, int nPixW, int nPixH)
{
  int w, h, n;
  int xx, yy, ww, hh;
  int pxx, pyy;
  ...
  for (h = 0; h < STATS_SIZE; h++)
	...
	(int) (yoff +
	       (double) (h +
			 1) * (double) gdk_pixbuf_get_height (im) /
      ...
      for (w = 0; w < STATS_SIZE; w++)
	{
	  ...
	  xx =
	    (int) (xoff +
		   (double) (w +
			     1) * (double) gdk_pixbuf_get_width (im) /
...
int *
gen_master_data (GdkPixbuf * im, int x, int y, int nPixW, int nPixH,
		 int order)
{
  int w, h;
  int xx, yy, ww, hh;
...
int
image_borders_n (struct IMAGE_INFO **image, struct PIC_DB *match, int n,
		 int cx, int cy, int maxx, int maxy)
{
  int x, y;
  for (y = cy - n; y <= cy + n; y++)
    {
...
int
guess_order (double val, int pixW, int pixH, int max_order)
{
  int order;
...
render ()
{
  ...
  if (copy_opt_data ())
	...
	g_object_unref (globals.in_im_scaled);
      ...
      /* scale the image */
      printf ("globals.cur_opt.pixW = %d\n", globals.cur_opt.pixW);
      printf ("globals.cur_opt.nPixW = %d\n", globals.cur_opt.nPixW);
      printf ("globals.cur_opt.pixH = %d\n", globals.cur_opt.pixH);
      printf ("globals.cur_opt.nPixH = %d\n", globals.cur_opt.nPixH);
      if (NULL ==
	  (globals.in_im_scaled =
	   gdk_pixbuf_scale_simple (globals.in_im,
				    globals.cur_opt.pixW *
				    globals.cur_opt.nPixW,
				    globals.cur_opt.pixH *
				    globals.cur_opt.nPixH,
  ...
  for (j = 0; j < nPixH * nPixW; j++)
      ...
      do
	{
	  for (i = 0;
	       i < MAX_MATCHES
	       && 1 == image_borders_n (image, matches[i], proximity, ww, hh,
					nPixW, nPixH); i++) /* do nothing */ ;
	  proximity--;
	}
