...
void
find_next (struct IMAGE_INFO **image, struct PIC_DB *db, int *col, int *row,
	   int nPixW, int nPixH)
{
  int r, c, cl;
  cl = (*col) + 1;
...
void
copy_image_RGB (GdkPixbuf * src, GdkPixbuf * dst, int col, int row)
{
...
GdkPixbuf *
render_image (struct IMAGE_INFO **image, int nPixW, int nPixH, int pixW,
	      int pixH)
{
  int i;
  double percent;
  ...
  for (hh = 0; hh < nPixH; hh++)
      ...
      for (ww = 0; ww < nPixW; ww++)
	{
	  ...
	  if (image[hh][ww].db->done == 1)
	    continue;
	  image[hh][ww].db->done = 1;

	  /* load the image */
	  gerror = NULL;
	  if (NULL ==
	      (im =
	       gdk_pixbuf_new_from_file (image[hh][ww].db->fname, &gerror)))
	    {
